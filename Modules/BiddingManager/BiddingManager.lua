...
function BiddingManager:CancelBid()
    ...
    Comms:Send(BIDDING_COMM_PREFIX, message, CONSTANTS.COMMS.DISTRIBUTION.WHISPER, self.auctioneer,
...
function BiddingManager:NotifyBonus()
    LOG:Trace("BiddingManager:NotifyBonus()")
    if not self.auctionInProgress then return end
    self.lastBid = CLM.L["bonus"]
    self.guiBid = true
    local message = BiddingCommStructure:New(CONSTANTS.BIDDING_COMM.TYPE.NOTIFY_BONUS, {})
    Comms:Send(BIDDING_COMM_PREFIX, message, CONSTANTS.COMMS.DISTRIBUTION.WHISPER, self.auctioneer,
        CONSTANTS.COMMS.PRIORITY.ALERT)
end

function BiddingManager:NotifyUpgrade()
    LOG:Trace("BiddingManager:NotifyUpgrade()")
    if not self.auctionInProgress then return end
    self.lastBid = CLM.L["upgrade"]
    self.guiBid = true
    local message = BiddingCommStructure:New(CONSTANTS.BIDDING_COMM.TYPE.NOTIFY_UPGRADE, {})
    Comms:Send(BIDDING_COMM_PREFIX, message, CONSTANTS.COMMS.DISTRIBUTION.WHISPER, self.auctioneer,
        CONSTANTS.COMMS.PRIORITY.ALERT)
end

function BiddingManager:NotifyOffspec()
    LOG:Trace("BiddingManager:NotifyOffspec()")
    if not self.auctionInProgress then return end
    self.lastBid = CLM.L["offspec"]
    self.guiBid = true
    local message = BiddingCommStructure:New(CONSTANTS.BIDDING_COMM.TYPE.NOTIFY_OFFSPEC, {})
    Comms:Send(BIDDING_COMM_PREFIX, message, CONSTANTS.COMMS.DISTRIBUTION.WHISPER, self.auctioneer,
        CONSTANTS.COMMS.PRIORITY.ALERT)
end
...
function BiddingManager:HandleAcceptBid(data, sender)
    ...
    if self.guiBid then
        ...
        UTILS.Debounce("CLM_BiddingManager_HandleAcceptBid", 0.5, function()
...
function BiddingManager:HandleDenyBid(data, sender)
    ...
    if self.guiBid then
        local value = self.lastBid or CLM.L["cancel"]
        UTILS.Debounce("CLM_BiddingManager_HandleDenyBid", 0.5, function()
            EventManager:DispatchEvent(
                CONSTANTS.EVENTS.USER_BID_DENIED,
                {
                    value = value,
                    reason = CONSTANTS.AUCTION_COMM.DENY_BID_REASONS_STRING[data:Reason()] or CLM.L["Unknown"]
                })
        end)
        LOG:Message(CLM.L["Your bid (%s) was denied: |cffcc0000%s|r"], value,
            CONSTANTS.AUCTION_COMM.DENY_BID_REASONS_STRING[data:Reason()] or CLM.L["Unknown"])
        self.guiBid = false
    end
end
