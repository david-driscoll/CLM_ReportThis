...
local function GenerateItemOptions(self)
    ...
    -- for _, key in ipairs({ CONSTANTS.SLOT_VALUE_TIER.SMALL, CONSTANTS.SLOT_VALUE_TIER.MEDIUM,
    --     CONSTANTS.SLOT_VALUE_TIER.LARGE, }) do
    --     options["value_" .. key] = {
    --         name = CONSTANTS.SLOT_VALUE_TIERS_GUI[key],
    --         type = "input",
    --         set = (function(i, v)
    ...
    if not CLM.OPTIONS.AuctionManagerBidSync:IAmTheAuctioneer() then
        options.note.width = 1.1
        UTILS.mergeDictsInline(options, {
            -- bid_stats_info = {
            --     name = "Info",
            --     desc = (function()
...
local function getResponseColumn(table, row)
    local row = table:GetRow(row)
    local col = #row.cols
...
local function rowColor(topBid, response, topBidIsUpgrade, topBidIsMain)
    local rollDifference = CLM.OPTIONS.AuctionManagerBidSync:GetRollDifference()

    local rowPoints = CLM.OPTIONS.AuctionManagerBidSync:TotalBid(response)
    local topPoints = CLM.OPTIONS.AuctionManagerBidSync:TotalBid(topBid)
    local color = { r = 1, g = 1, b = 1, a = 1 }

    if (topPoints - rowPoints) > rollDifference then
        color = { r = 1, g = 0, b = 0, a = 1 }
    elseif not response.roll then
...
local function BuildBidRow(item, name, response, roster, namedButtonMode)
    ...
    local topPoints, isUpgradeOrBonus, isMain = CLM.OPTIONS.AuctionManagerBidSync:GetTopBid(item)
    local bidInfo = CLM.OPTIONS.AuctionManagerBidSync:GetBidInfo(item, name, response)
    local rowColorValue = rowColor(topPoints, response, isUpgradeOrBonus, isMain)

    if isUpgradeOrBonus and isMain and not bidInfo.isMain then
        highlight = highlightAlt
    end

    if bidInfo.isOffspec then
...
function AuctionManagerGUI:Refresh()
    ...
    if CLM.OPTIONS.AuctionManagerBidSync:IAmTheAuctioneer() then
        self.top:SetHeight(484)
    else
        self.top:SetHeight(414)
    end
end
